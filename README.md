# SQL Analysis: Revenue and Performance Report
<img width="633" height="355" alt="image" src="https://github.com/user-attachments/assets/915876de-cb58-4b03-9703-5ad76090a023" />

## Objective
Produce insights to support strategic growth and revenue improvements

## Tasks
- calculate revenue by treatment/doctor
- segment high-value patients
- simulate pricing changes- track changes over time
- flag and update anomalies

## Tools
- SQL (for querying, aggregating, and analyzing the dataset)
- BigQuery (for handling datasets and performing queries)
- ERD (for database schema)
- PowerPoint (for data presentation key insights for stakeholders)

## Deliver
- SQL scripts with comments
- revenue summary
- one-slide business insight pitch


- <img width="635" height="356" alt="image" src="https://github.com/user-attachments/assets/b4281624-27d9-4ef8-a902-2219b5626847" />
---
- <img width="624" height="344" alt="image" src="https://github.com/user-attachments/assets/d9740b10-04fb-4657-9fd5-9bbfa0f6f56e" />
---
- <img width="158" height="152" alt="image" src="https://github.com/user-attachments/assets/1f33a537-c40f-4c44-864f-139fccd611f4" />
---
- <img width="165" height="210" alt="image" src="https://github.com/user-attachments/assets/0f59e67e-ed5f-469a-8d12-9dfd2b6c9517" />
---
- <img width="108" height="106" alt="image" src="https://github.com/user-attachments/assets/19d6e6eb-abc5-44b5-b58e-fa290be5a1e9" />
---
- <img width="524" height="294" alt="image" src="https://github.com/user-attachments/assets/c415a9d0-eb7e-4bf7-8869-be26dce2bfd5" />


